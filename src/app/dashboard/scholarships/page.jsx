...
export default function ScholarshipsPage() {
    ...
    useEffect(() => {
        applyFilters();
    }, [searchTerm, locationFilter, deadlineFilter, scholarships]);
    ...
    const applyFilters = () => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(scholarship =>
                ...
                scholarship.provider.toLowerCase().includes(lowerSearch) ||
                scholarship.description.toLowerCase().includes(lowerSearch) ||
                (scholarship.location && scholarship.location.toLowerCase().includes(lowerSearch))
            );
        }
        ...
        if (locationFilter !== 'all') {
            filtered = filtered.filter(scholarship => 
                scholarship.location && scholarship.location.toLowerCase() === locationFilter.toLowerCase()
            );
        }
        ...
        if (deadlineFilter !== 'all') {
            ...
            switch (deadlineFilter) {
                case 'past':
                    filtered = filtered.filter(scholarship => scholarship.deadline !== null && isPast(new Date(scholarship.deadline)));
                    break;
                case 'upcoming':
                    filtered = filtered.filter(scholarship => scholarship.deadline !== null && !isPast(new Date(scholarship.deadline)));
                    break;
                case 'thisMonth':
                    filtered = filtered.filter(scholarship => {
                        if (scholarship.deadline === null) return false;
                        const deadlineDate = new Date(scholarship.deadline);
                        return !isPast(deadlineDate) && deadlineDate <= oneMonth;
                    });
                    break;
                case 'noDeadline':
                    filtered = filtered.filter(scholarship => scholarship.deadline === null);
                    break;
            }
        }
            ...
            {filteredScholarships.length === 0 ? (
                ...
                >
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 3, textAlign: 'center', maxWidth: 400 }}>
                        {searchTerm || locationFilter !== 'all' || deadlineFilter !== 'all'
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <CardMedia
                                                component="img"
                                                height={120}
                                                image={scholarship.image_url || '/images/placeholder-scholarship.jpg'}
                                                ...
                                                sx={{
                                                    ...
                                                    alignItems: 'center',
                                                    position: 'relative',
                                                    bgcolor: scholarship.image_url ? 'transparent' : 'primary.main',
                                                }}
                                            />
                                            ...
                                            <CardContent sx={{ flexGrow: 1, pt: 2 }}>
                                                ...
                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                                                    ...
                                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                        ...
                                                        >
                                                            {scholarship.deadline ? format(new Date(scholarship.deadline), 'MMM d, yyyy') : 'No deadline'}
                                                        </Typography>
                                                    </Box>
                                                </Box>
                            ...
                            {selectedScholarship && (
                                <Box sx={{ mb: 3, mt: 1 }}>
                                    ...
                                    <Typography variant="body2" color="text.secondary" gutterBottom>
                                        Organization: {selectedScholarship.provider}
                                    </Typography>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mt: 1 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            <LocationIcon sx={{ fontSize: 16, mr: 0.5, color: 'text.secondary' }} />
                                            <Typography variant="body1" fontWeight="medium" color="text.secondary">
                                                {selectedScholarship.location || 'Global'}
                                            </Typography>
                                        </Box>
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            <CalendarIcon sx={{ fontSize: 16, mr: 0.5, color: 'text.secondary' }} />
                                            <Typography variant="body2" color="text.secondary">
                                                Deadline: {selectedScholarship.deadline ? format(new Date(selectedScholarship.deadline), 'MMMM d, yyyy') : 'No deadline'}
                                            </Typography>
                                        </Box>
                                    </Box>
                                </Box>
